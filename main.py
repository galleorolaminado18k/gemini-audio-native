...
def chat():
    try:
        ...
        # Guardar en Google Sheets (texto + audio_base64)
        if gs_ready and sheet is not None:
            try:
                sheet.append_row([user_text, "[AUDIO Gemini]", audio_base64])
            except Exception as e_sheet:
                print(f"⚠️ No se pudo escribir en Sheets: {e_sheet}")

        # Guardar en cache y servir por URL
        audio_id = str(uuid4())
        audio_cache[audio_id] = audio_data
        audio_url = f"{_public_base_url()}/audio/{audio_id}.ogg"

        return jsonify({
            "text_response": "[Gemini TTS Audio generado]",
